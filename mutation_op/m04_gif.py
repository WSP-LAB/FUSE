...
class mOP(MutationOP):
  ...
  __seed_dependency__ = __exclusion_op__.keys()#["php"] # seed file dependency for operation
  ...
  def operation(self, output, seed_file, resource_file=None):
      ...
      output['filename'] = filename + '_M4GIF'
      if '.' not in output['fileext']:
        output['fileext'] = 'gif'
      else:
        output['fileext'] = output['fileext'].rsplit('.',1)[0]+'.gif'
